...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            var type = parts[2];
            bool weighted = parts[3].ToLower().Equals("true") ? true : false;
            BaseGradeBook gradeBook = null;
            if (type == "standard")
                gradeBook = new StandardGradeBook(name.ToLower(), weighted);
            else if (type == "ranked")
                gradeBook = new RankedGradeBook(name.ToLower(), weighted);
            else
            {
